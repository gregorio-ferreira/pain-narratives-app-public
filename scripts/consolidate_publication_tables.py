...
CSV_FILES_CONFIG = [
    ...
    (
        "questionnaire_merged_results.csv",
        "Data_QuestMerged",
...
ADDITIONAL_DATA_FILES = [
    (
        "data/excel_id_to_db_mapping.csv",
        "Map_ExcelToDB",
...
def consolidate_csvs(output_dir: Path, output_file: Path, project_root: Path, verbose: bool = True) -> bool:
    """
            ...
            metadata = pd.DataFrame(
                [
                    {"Index": "", "Sheet Name": "", "Source File": "", "Description": "", "Status": ""},
                    {
                        ...
                        "Source File": str(datetime.now()),
                        "Description": "Publication tables consolidated",
                        "Status": "",
                    },
                ]
            )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Consolidate publication CSV files into Excel workbook")
    ...
    success = consolidate_csvs(
        output_dir=output_dir, output_file=output_file, project_root=project_root, verbose=not args.quiet
    )
