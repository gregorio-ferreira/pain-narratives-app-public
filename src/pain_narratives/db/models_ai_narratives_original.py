"""
SQLModel models for the ``ai_narratives_original`` schema.

Authoritative replacement for the lost ``pain_narratives_acm_202512`` schema.
Holds real patient data, expert evaluations, and multi-model LLM synthetic data
for the rebuttal + extended analyses across GPT-5, DeepSeek-R1 and Sonnet 4.5.

Tables:
- ``narratives``               canonical narrative table (source of narrative_id)
- ``real_patient_*``           real patient ground truth (152 rows each)
- ``expert_*``                 expert Likert agreement feedback
- ``llm_*_results``            LLM synthetic outputs (composite PK on model + run_number)
- ``sus_usability_results``    System Usability Scale responses
"""
...
SCHEMA_NAME = "ai_narratives_original"


# =============================================================================
# Canonical narrative table (source of narrative_id)
# =============================================================================


class Narratives(SQLModel, table=True):
    """Canonical narrative store.

    Each narrative_text appears once. narrative_id is auto-assigned. All other
    tables refer to this id (kept as ``participant_id`` in column names for
    back-compat with the existing analysis notebooks).
    """

    __tablename__ = "narratives"  # type: ignore
    __table_args__ = {"schema": SCHEMA_NAME}

    narrative_id: Optional[int] = Field(default=None, primary_key=True)
    narrative_hash: str = Field(max_length=64, unique=True)
    narrative_text: str
    word_count: Optional[int] = None
    response_id: Optional[str] = None  # Qualtrics response id, kept for provenance
...
class LLMDimensionEvaluation(SQLModel, table=True):
    ...
    run_number: int = Field(primary_key=True)
